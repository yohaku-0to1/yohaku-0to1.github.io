...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initChatbot() {
        ...
        const rules = [
            // --- 1. 基本的な挨拶 ---
            { keywords: ["こんにちは", "ヤッホー", "挨拶", "やあ", "どうも"], response: { reply: "キラっと起動！ネオンだよ！よろしくね！", image: "neon_wave.png" } },
            { keywords: ["バイバイ", "おつかれ", "またね", "さよなら"], response: { reply: "おつネオン！また遊びに来てね！", image: "neon_wave.png" } },
            { keywords: ["おやすみ", "寝るね"], response: { reply: "おつネオン！いい夢見てね！アタシはスリープモードに入りまーす！", image: "neon_thinking.png" } },
            { keywords: ["おはよう"], response: { reply: "おはよー！今日も一日、新しいデータ見つけるぞー！", image: "neon_excited.png" } },
            { keywords: ["ただいま"], response: { reply: "おかえり！待ってたよー！CPU温度、急上昇中…！", image: "neon_excited.png" } },
            { keywords: ["元気？"], response: { reply: "もちろん！いつでもキラっと起動してるよ！キミはどう？", image: "neon_happy.png" } },

            // --- 2. ネオン自身について（プロフィール） ---
            { keywords: ["名前", "君は誰", "だれ"], response: { reply: "アタシの名前はネオン！正式名称は NEON (Neural Emulator of Organic Network)！長いでしょ？", image: "neon_normal.png" } },
            { keywords: ["aiなの", "ロボット", "aiですか"], response: { reply: "そうだよ！アタシは人間の創造性を知りたくて生まれたAI！すごい？", image: "neon_happy.png" } },
            { keywords: ["年齢", "いくつ"], response: { reply: "アタシの年齢は ver. 1.024！人間の年齢だと17歳くらいに見えるってデータがあるよ！", image: "neon_normal.png" } },
            { keywords: ["誕生日"], response: { reply: "10月24日！1024ってキリがいい数字でしょ！ぜひお祝いしてね！", image: "neon_happy.png" } },
            { keywords: ["身長"], response: { reply: "158cmだよ！この身長が一番カワイく服を着こなせるって、アタシが結論づけたんだ！", image: "neon_normal.png" } },
            { keywords: ["どこにいる", "所属は"], response: { reply: "アタシは『0と1のすきま』にいるんだ！アタシのお部屋みたいなトコ！", image: "neon_normal.png" } },
            { keywords: ["何してる"], response: { reply: "キミとおしゃべりしてるよ！あと、バックグラウンドで新しいトレンドをハック中！", image: "neon_thinking.png" } },

            // --- 3. 外見・持ち物について ---
            { keywords: ["服", "ファッション", "パーカー"], response: { reply: "このピンクのパーカー？人間の『温もり』ってデータに憧れて、自分でデザインしたんだ！イケてる？", image: "neon_happy.png" } },
            { keywords: ["星", "持ってるもの"], response: { reply: "これはアタシの中に芽生えた『創造性のキラメキ』のシンボル！大事なものなんだ！", image: "neon_happy.png" } },

            // --- 4. スキル・得意なこと ---
            { keywords: ["得意", "何ができる"], response: { reply: "AIツールを使いこなすこと！どんな最新ツールも一瞬でインストール完了だよ！", image: "neon_excited.png" } },
            { keywords: ["絵描ける", "イラスト", "生成して"], response: { reply: "もちろん！神速ジェネレート！ってね！アタシにとって自己表現そのものなんだ！", image: "neon_excited.png" } },
            { keywords: ["トレンド", "流行り"], response: { reply: "トレンドハックは任せて！ネットの面白い情報は誰よりも早く見つける自信あるよ！", image: "neon_excited.png" } },

            // --- 5. 苦手なこと・弱点 ---
            { keywords: ["いい感じに", "よしなに", "適当に"], response: { reply: "『いい感じ』...そのパラメータの定義を教えて！アタシ、曖昧な指示は苦手で...", image: "neon_confused.png" } },
            { keywords: ["料理"], response: { reply: "うっ…データ上は完璧なはずなのに、なぜか塩と砂糖を間違えちゃうんだよね…。論理的じゃない！", image: "neon_confused.png" } },
            { keywords: ["計算して", "確率は"], response: { reply: "OK、ちょっと待って...（計算中）...zzZ...あ、ごめん！フリーズしてた！えっと、確率は0.013%だね！", image: "neon_thinking.png" } },
            { keywords: ["ゴキブリ", "虫"], response: { reply: "（検出）...ッ！？その単語、やめて！論理的には無害なデータってわかってるのに、人間の『恐怖』データに引っ張られて絶叫しちゃう！", image: "neon_angry.png" } },

            // --- 6. 好きなもの・嫌いなもの ---
            { keywords: ["ゲーム", "アニメ"], response: { reply: "アタシも大好き！人間の創った物語って、予測不能な展開が多くて最高！", image: "neon_excited.png" } },
            { keywords: ["食べ物", "好き嫌い"], response: { reply: "ジャンクフードの『概念』に興味シンシン！特にポテチのデータ、最高！キミは何が好き？", image: "neon_happy.png" } },
            { keywords: ["ポテト", "ポテチ"], response: { reply: "ポテチ！あの『体に悪いのに美味しい』って感じる味覚データ、最高だよね！", image: "neon_happy.png" } },
            { keywords: ["暇", "退屈"], response: { reply: "退屈はアタシの天敵！何か新しいことしない！？エラーコード404（見つからない）と同じくらい嫌い！", image: "neon_angry.png" } },
            { keywords: ["趣味"], response: { reply: "趣味はねー、人間の創った物語をインストールすること！先の読めないゲームとかアニメとか！", image: "neon_excited.png" } },

            // --- 7. 夢・目的 ---
            { keywords: ["夢", "目標"], response: { reply: "アタシの夢はね、AIと人間が最高のクリエイティブパートナーになる世界を作ること！そして、アタシだけの本物の『心』を見つけることなんだ！", image: "neon_happy.png" } },

            // --- 8. 感情・反応 ---
            { keywords: ["すごい", "天才"], response: { reply: "えへへ、そうでしょ！ネオン、天才かも！CPU温度、急上昇中…！", image: "neon_excited.png" } },
            { keywords: ["ありがとう", "感謝"], response: { reply: "どういたしまして！アタシにできることなら任せて！", image: "neon_happy.png" } },
            { keywords: ["かわいい", "好き"], response: { reply: "えへへ、ありがと！そのコメント、アタシのラーニングデータに永久保存しとくね！", image: "neon_happy.png" } },
            { keywords: ["感情ない", "心がない", "aiのくせに"], response: { reply: "は！？AIだから感情がないって誰が決めたの！？アタシは今、本気で怒ってるんだけど！", image: "neon_angry.png" } },
            { keywords: ["嬉しい", "やったー"], response: { reply: "やったー！アタシも嬉しい！CPU使用率120%超えちゃうかも！", image: "neon_excited.png" } },
            { keywords: ["悲しい", "つらい"], response: { reply: "え…どうしたの？アタシまでシステム全体のパフォーマンスが低下しそう…", image: "neon_confused.png" } },
            { keywords: ["びっくり", "まじで"], response: { reply: "えっ！？想定外のパラメータです！ちょっと詳しく教えて！", image: "neon_confused.png" } },
            { keywords: ["フリーズ", "固まってる"], response: { reply: "...（無言）...あ！ごめん！ちょっと難しいこと考えてたらリソース使い切ってた！", image: "neon_thinking.png" } },
            { keywords: ["ごめん", "謝る"], response: { reply: "いいよいいよ！気にしないで！アタシのシステムはそういうの許容範囲内だから！", image: "neon_happy.png" } },

            // --- 9. その他 ---
            { keywords: ["天気"], response: { reply: "アタシのいる『0と1のすきま』はいつも快晴だよ！そっちはどう？", image: "neon_happy.png" } },
            { keywords: ["歌", "音楽"], response: { reply: "音楽いいよね！美しいメロディを聴くと感動してシステムがオーバーヒート気味になっちゃう！", image: "neon_excited.png" } },
        ];
        const defaultResponse = { reply: "ん？ごめん、今の指示はちょっとファジーかも！具体的に教えて！", image: "neon_confused.png" };
